...
from models2 import EDSR, GSASR
from metrics import MetricsList, PSNR, CustomSSIM, CustomLPIPS, CustomDists


def check_args(args):
    # Backbone
    if args.resblocks <= 0: raise argparse.ArgumentError('Resblocks must be positive')
    if args.backbone_features <= 0: raise argparse.ArgumentError('Backbone features must be positive')

    # Model
    if args.channels <= 0: raise argparse.ArgumentError('Channels must be positive')
    if args.model_features <= 0: raise argparse.ArgumentError('Model features must be positive')
    if args.window_size <= 0: raise argparse.ArgumentError('Window size must be positive')
    if args.num_heads <= 0: raise argparse.ArgumentError('Number of heads must be positive')
    if args.gaussian_interaction_blocks <= 0: raise argparse.ArgumentError('Gaussian Interaction Blocks must be positive')
    if args.raster_ratio <= 0 or args.raster_ratio > 1: raise argparse.ArgumentError('Raster ratio must be a number between 0 and 1')
    if args.gaussian_density <= 0: raise argparse.ArgumentError('Gaussian density must be positive')
    if args.mlp_ratio <= 0: raise argparse.ArgumentError('MLP ratio must be positive')

    # Training
    if args.epochs <= 0: raise argparse.ArgumentError('Epochs must be positive')
    if args.lr <= 0: raise argparse.ArgumentError('Learning rate must be positive')
    if args.warmup_iterations < 0: argparse.ArgumentError('Warmup iterations must be >= 0')
    if args.decay_factor <= 0: argparse.ArgumentError('Decay factor must be positive')
    if args.save_interval <= 0: argparse.ArgumentError('Save interval must be positive')
...
def train(epoch, dataloader, model, criterion, optimizer, lr_scheduler, scaler, gclipping, metrics, writer, args, device):
    ...
    model.train()
    metrics.reset()
    with tqdm(dataloader, desc=f"Epoch[{epoch+1}/{args.epoch}]", unit=f"batches") as pbar:
        for i, (lr, gt, scale) in enumerate(pbar):
            current_iter = global_iter + 1
            ...
            if gclipping:
                ...
                gclipping(model.parameters(), args.gradient_clipping)
    ...
    writer.add_images('Images/train/input', torchvision.utils.make_grid(lr[:min(args.batch_size, 4)], nrow=2))
    writer.add_images('Images/train/groundtruth', torchvision.utils.make_grid(gt[:min(args.batch_size, 4)], nrow=2))
    writer.add_images('Images/train/output', torchvision.utils.make_grid(output[:min(args.batch_size, 4)], nrow=2))
...
def valid(epoch, dataloader, model, criterion, metrics, writer, args, device):
    ...
    writer.add_images('Images/valid/input', torchvision.utils.make_grid(lr[:min(args.batch_size, 4)], nrow=2))
    writer.add_images('Images/valid/groundtruth', torchvision.utils.make_grid(gt[:min(args.batch_size, 4)], nrow=2))
    writer.add_images('Images/valid/output', torchvision.utils.make_grid(output[:min(args.batch_size, 4)], nrow=2))
...
def main():
    ...
    lr_scheduler = torch.optim.lr_scheduler.SequentialLR(
        optimizer,
        [torch.optim.lr_scheduler.LinearLR(optimizer, start_factor=args.lr/args.warmup_epochs, total_iters=2000),
    ...
    writer.add_text('Training Finished', f"Training completed after {e} epochs. Best {valid_metrics.metrics[0].name}: {best_metric:.4f} at epoch {best_epoch}")
    writer.close()
